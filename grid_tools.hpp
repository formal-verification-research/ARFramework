...
using refinement_strategy_return_t = std::set<region, region_less_compare>;
...
using norm_function_type_t = std::function<long double(point const &)>;
...
enum class VERIFICATION_RETURN { SAFE, UNSAFE, UNKNOWN };

using verification_engine_return_t = std::pair<VERIFICATION_RETURN, point>;
using verification_engine_type_t =
    std::function<verification_engine_return_t(region const &)>;

region snapToDomainRange(region const &, /* region */
                         region const &);

point snapToDomainRange(point const &, /* point */
...
// excessive copying
point enforceSnapDiscreteGrid(point const & /* p */,
...
abstraction_strategy_return_t centralPointRegionAbstraction(region const &);
...
struct AllValidDiscretizedPointsAbstraction {
  AllValidDiscretizedPointsAbstraction(point /* knownValidPoint */,
                                       point /* granularity */);
  abstraction_strategy_return_t operator()(grid::region const &);
  static unsigned long long getNumberValidPoints(grid::region const &,
                                                 grid::point const &,
                                                 grid::point const &);
  unsigned long long getNumberValidPoints(grid::region const &);
  static std::pair<bool, grid::point>
  findValidPointInRegion(grid::region const &, grid::point const &,
                         grid::point const &);
  std::pair<bool, grid::point> findValidPointInRegion(grid::region const &);

private:
  bool enumerateAllPoints(abstraction_strategy_return_t &, grid::point const &,
                          std::size_t, grid::region const &);
...
dim_selection_strategy_return_t randomDimSelection(region const &, std::size_t);
dim_selection_strategy_return_t largestDimFirst(region const &, std::size_t);
...
public:
  ...
  dim_selection_strategy_return_t operator()(region const &, std::size_t);
...
struct IntellifeatureDimSelection {
  IntellifeatureDimSelection(std::vector<point> const &, norm_function_type_t,
                             std::size_t);
...
struct ModifiedFGSMWithFallbackRegionAbstraction {
  ModifiedFGSMWithFallbackRegionAbstraction(
      ...
      region_abstraction_strategy_t const &, point const &,
...
struct RPlusFGSMAbstraction {
  RPlusFGSMAbstraction(
      std::size_t, std::function<point(point const &)> const & /* gradient */,
      point const & /* granularity */, double);
  abstraction_strategy_return_t operator()(region const &);
...
struct HierarchicalDimensionRefinementStrategy {
  HierarchicalDimensionRefinementStrategy(
      dimension_selection_strategy_t const &, unsigned /* dimension divisor */,
      unsigned /* number of dimensions to subdivide */);
  refinement_strategy_return_t operator()(region const &);

private:
  bool enumerateAllRegions(refinement_strategy_return_t &, region const &,
                           unsigned,
...
template <class T> grid::point constVecMult(T const &t, grid::point const &p) {
...
std::ostream &operator<<(std::ostream &os, grid::VERIFICATION_RETURN const &);

template <class ArrayLike>
std::vector<std::size_t> getSortedIndices(ArrayLike const &arr,
                                          bool asc = true) {
  std::vector<std::size_t> retInd(arr.size());
  std::iota(retInd.begin(), retInd.end(), 0u);
  std::sort(retInd.begin(), retInd.end(),
            [&arr, asc](std::size_t a, std::size_t b) {
