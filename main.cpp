...
int main(int argc, char *argv[]) {
  ...
  std::vector<tensorflow::Flag> flag_list = {
      tensorflow::Flag(
          "graph", &graph,
          "path to protobuf graph to be executed - root_dir/graph"),
      ...
      tensorflow::Flag("gradient_layer", &gradient_layer,
                       "name of the gradient layer (optional - used for FGSM)"),
      tensorflow::Flag(
          "granularity", &granularity,
          "use this option if all dimensions share a discrete range"),
      tensorflow::Flag(
          "verification_radius", &verification_radius,
          "'radius' of hyperrectangle within which safety is to be verified"),
      tensorflow::Flag(
          "initial_activation", &initial_activation,
          "initial tested activation - root_dir/initial_activation.pb"),
      tensorflow::Flag("root_dir", &root_dir,
                       "root directory holding resources such as the graph, "
                       "intial_activation, class_averages, etc protobufs"),
      tensorflow::Flag("class_averages", &class_averages,
                       "the class averages of the training data (optional - "
                       "used for Intellifeature)"),
      tensorflow::Flag("label_proto", &label_proto,
                       "protocol buffer of label image corresponding with "
                       "initial activation (optional - used in FGSM)"),
      tensorflow::Flag("label_layer", &label_layer,
                       "name of input label layer"),
      tensorflow::Flag("fgsm_balance_factor", &fgsm_balance_factor_opt,
                       "Balance factor for modified FGSM algorithm (ratio "
                       "dimensions fgsm/random)"),
      tensorflow::Flag("num_threads", &num_threads_str,
                       "number of threads to use"),
      tensorflow::Flag(
          "num_abstractions", &num_abstractions_str,
          "number of points to use as abstractions for each region"),
      tensorflow::Flag("output_dir", &output_dir,
                       "directory where adversarial examples and other output "
                       "should be saved"),
      tensorflow::Flag("refinement_dim_selection", &refinement_dim_selection,
                       "strategy to use for hierarchical dimension refinement"),
      tensorflow::Flag("modified_fgsm_dim_selection",
                       &modified_fgsm_dim_selection,
                       "dimension selection strategy to use for modified FGSM"),
      tensorflow::Flag("granularity_array", &granularity_array_str,
                       "comma separated values specifying the granularity"),
      tensorflow::Flag(
  ...
  if (!init_act_tensor_status_pair.first) {
    LOG(ERROR) << "Could not read initial activation protobuf: "
               << initial_activation_path;
    exit(1);
  }

  auto init_act_tensor = init_act_tensor_status_pair.second;
  auto init_act_point = graph_tool::tensorToPoint(init_act_tensor);
  ...
  auto logits_init_activation = gm.feedThroughModel(
      retFeedDict, &graph_tool::parseGraphOutToVector, {output_layer});
  if (!gm.ok()) {
  ...
  if (hasLabelProto) {
    auto label_tensor_path = tensorflow::io::JoinPath(root_dir, label_proto);
  ...
  */
  ...
  if (granularity_array_str != "") {
    ...
    std::cout << "using itemized granularity\n";
    std::stringstream tmp_stream(granularity_array_str);
    std::string word;
    while (std::getline(tmp_stream, word, ',')) {
      granularity_parsed.push_back(std::stod(word));
    }
  } else {
    std::cout << "using global granularity\n";
    double granularityVal =
        granularityProvided ? atof(granularity.c_str()) : 1.0;
    std::fill_n(std::back_inserter(granularity_parsed), flattenedNumDims,
                granularityVal);
  }
  if (granularity_parsed.size() != flattenedNumDims) {
    LOG(ERROR) << "wrong number of provided granularity dimensions";
    exit(1);
  }
  ...
  */
  ...
  if (hasAveragesProto && (refinement_dim_selection == "intellifeature" ||
                           modified_fgsm_dim_selection == "intellifeature")) {
    ...
    auto averages =
        graph_tool::tensorToPoints(class_averages_proto_pair.second);
    if (refinement_dim_selection == "intellifeature") {
      ...
      dimension_selection_strategy =
          grid::IntellifeatureDimSelection(averages, &grid::l2norm, orig_class);
    }
    if (modified_fgsm_dim_selection == "intellifeature") {
      std::cout
          << "Using Intellifeature for modified FGSM dimension selection\n";
  ...
  auto canUseGradient = hasGradientLayer && hasLabelProto && hasLabelLayer;

  if (canUseGradient) {
    std::cout << "Using Modified FGSM: " << fgsm_balance_factor << "\n";
    auto label_tensor_path = tensorflow::io::JoinPath(root_dir, label_proto);
    auto label_tensor_pair =
        GraphManager::ReadBinaryTensorProto(label_tensor_path);
    if (!label_tensor_pair.first) {
    ...
    auto label_tensor = label_tensor_pair.second;
    auto grad_func = [&, label_tensor_copy = label_tensor](
                         grid::point const &p) -> grid::point {
      auto createGradientFeedDict = [&]() -> graph_tool::feed_dict_type_t {
        auto p_tensor = graph_tool::pointToTensor(p, batch_input_shape);
        return {{input_layer, p_tensor}, {label_layer, label_tensor_copy}};
      };
      auto retVal = gm.feedThroughModel(createGradientFeedDict,
                                        graph_tool::parseGraphOutToVector,
                                        {gradient_layer});
      if (!gm.ok())
    ...
    if (modified_fgsm_dim_selection == "gradient_based") {
      std::cout << "Using gradient-based dimension selection for modified FGSM "
                   "abstraction strategy\n";
      modified_fgsm_selection_strategy =
          grid::GradientBasedDimensionSelection(grad_func);
    }
    abstraction_strategy = grid::ModifiedFGSMWithFallbackRegionAbstraction(
        num_abstractions, grad_func, modified_fgsm_selection_strategy,
        grid::RandomPointRegionAbstraction(2u), granularity_parsed,
        fgsm_balance_factor);
    if (refinement_dim_selection == "gradient_based") {
      std::cout << "Using gradient-based dimension selection strategy for "
                   "partitioning\n";
      dimension_selection_strategy =
          grid::GradientBasedDimensionSelection(grad_func);
    }
  ...
  grid::region_refinement_strategy_t refinement_strategy =
      grid::HierarchicalDimensionRefinementStrategy(
          dimension_selection_strategy, 2u, 2u);

  auto all_valid_discretization_strategy =
      grid::AllValidDiscretizedPointsAbstraction(
          graph_tool::tensorToPoint(init_act_tensor), granularity_parsed);
  ...
  auto discrete_search_attempt_threshold_func = [&](grid::region const &r) {
    return all_valid_discretization_strategy.getNumberValidPoints(r) <
           discrete_search_attempt_threshold;
  };

  auto isPointSafe = [&](grid::point const &p) {
    auto logits_out = gm.feedThroughModel(
        std::bind(graph_tool::makeFeedDict, input_layer, p, batch_input_shape),
        &graph_tool::parseGraphOutToVector, {output_layer});
    if (!gm.ok())
      LOG(ERROR) << "GM Error in isPointSafe";
    auto class_out = graph_tool::getClassOfClassificationVector(logits_out);
  ...
  auto verification_engine = grid::DiscreteSearchVerificationEngine(
      discrete_search_attempt_threshold_func, all_valid_discretization_strategy,
      isPointSafe);
  ...
  ARFramework arframework(gm, domain_range, init_act_point, granularity_parsed,
                          orig_region, isPointSafe, verification_engine,
                          abstraction_strategy, refinement_strategy,
                          greedy_termination);
  ...
  auto report_function = [&](grid::point const &adv_exp) {
    ...
    auto adv_exp_tensor_proto =
        tensorflow::tensor::CreateTensorProto(tmp, tmp_size);
    auto classification =
        graph_tool::getClassOfClassificationVector(gm.feedThroughModel(
            std::bind(graph_tool::makeFeedDict, input_layer, adv_exp,
                      batch_input_shape),
            &graph_tool::parseGraphOutToVector, {output_layer}));
    if (!gm.ok())
      LOG(ERROR) << "GM: error in report function";
    std::stringstream file_name;
    file_name << timestamp << "_" << index << "_" << orig_class << "_"
              << classification << ".pb";
    ++index;
    auto file_path = tensorflow::io::JoinPath(output_dir, file_name.str());
    auto write_status = WriteBinaryProto(tensorflow::Env::Default(), file_path,
